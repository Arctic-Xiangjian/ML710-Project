...
model_3 = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=5)
model_4 = timm.create_model('vit_base_patch16_224', pretrained=True, num_classes=5)


wandb.login(key=WANDB_API)
run = wandb.init(project='ml710_project', entity='arcticfox', name='data_ours_4gpu'+'_'+'vit'+'_'+datetime.now().strftime('%Y%m%d_%H%M%S'), job_type="training",reinit=True)
...
def train(model_groups, train_dataloader, optimizers, criterion, num_epochs, device_groups):
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for batch_idx, (data, target) in enumerate(train_dataloader):
            data_group1, data_group2 , data_group3, data_group4 = data.split(data.shape[0] // 4, dim=0)
            ...
            output_group4 = output_group4.to(output_group1.device)
            output = torch.cat([output_group1, output_group2, output_group3, output_group4], dim=0)
...
optimizer_group4 = optim.AdamW(model_4.parameters(), lr=1e-3)

optimizers = [optimizer_group1, optimizer_group2, optimizer_group3, optimizer_group4]

criterion = nn.CrossEntropyLoss()

train([model_1, model_2, model_3, model_4], train_dataloader, optimizers, criterion, EPOCHS, GPU_IDS)