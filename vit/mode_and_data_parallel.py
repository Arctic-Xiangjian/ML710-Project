...
from parallel.model_data_parallel_class import ViTModel_Data_Parallel
...
device_groups = [['cuda:0','cuda:1'], ['cuda:2', 'cuda:3']]
model_name = 'vit_large_patch16_224'
num_classes = 5

model_parallel_group1 = ViTModel_Data_Parallel(model_name, num_classes, device_groups[0])
model_parallel_group2 = ViTModel_Data_Parallel(model_name, num_classes, device_groups[1])
...
def train(model_groups, train_dataloader, optimizers, criterion, num_epochs, device_groups):
    for epoch in tqdm(range(num_epochs)):
        ...
        for batch_idx, (data, target) in enumerate(train_dataloader):
            data_group1, data_group2 = data.split(data.shape[0] // 2, dim=0)

            data_group1 = data_group1.to(device_groups[0][0])
            data_group2 = data_group2.to(device_groups[1][0])
            ...
            output = torch.cat([output_group1, output_group2], dim=0)
...
optimizer_group1 = optim.AdamW(model_parallel_group1.parameters(), lr=1e-3)
optimizer_group2 = optim.AdamW(model_parallel_group2.parameters(), lr=1e-3)

optimizers = [optimizer_group1, optimizer_group2]

criterion = nn.CrossEntropyLoss()

train([model_parallel_group1, model_parallel_group2], train_dataloader, optimizers, criterion, EPOCHS, device_groups)