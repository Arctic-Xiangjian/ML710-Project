...
model = nn.DataParallel(model_BASE, device_ids=GPU_IDS)

wandb.login(key=WANDB_API)
run = wandb.init(project='ml710_project', entity='arcticfox', name='data_pytorch1_3gpu'+'_'+'vit'+'_'+datetime.now().strftime('%Y%m%d_%H%M%S'), job_type="training",reinit=True)
